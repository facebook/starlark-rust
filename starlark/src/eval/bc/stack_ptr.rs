...
impl<'v, 's> BcStackPtr<'v, 's> {
    fn new(slice: &'s mut [MaybeUninit<Value<'v>>]) -> BcStackPtr<'v, 's> {
        BcStackPtr {
    ...
    /// Alloca the stack. Return a pointer to the bottom of the stack.
    pub(crate) fn alloca<'a, R>(
        eval: &mut Evaluator<'v, 'a>,
        max_stack_size: u32,
        k: impl FnOnce(&mut Evaluator<'v, 'a>, BcStackPtr<'v, '_>) -> R,
    ) -> R {
        eval.alloca_uninit::<Value, R, _>(max_stack_size as usize, |stack, eval| {
            // Do not initialize the stack: generated bytecode always writes values before reading.
            k(eval, BcStackPtr::new(stack))
        })
    }
