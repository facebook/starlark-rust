...
fn test_display_debug() {
    ...
    assert_eq!(
        format!("{:?}", val),
        "Value(TupleGen { content: [Value(ListGen(List { content: Cell { value: ValueType(Value(Array { len: 2, capacity: 4, iter_count: 0, content: [Value(1), Value(2)] })) } })), Value(\"test\"), Value(StarlarkBool(true))] })"
    );
